...
for (( i = 0; i < 6; i++ )); do # 5 runs
    ...
    ./runner -n 2 -b 192 --num-epochs 90 --num-classes 100 --mode train_val --amp --dllogger-log /cifar100/MxNet/run_"${i}"/run_"${i}".log --workspace /cifar100/MxNet/run_"${i}"/ --data-backend mxnet --image-shape 3,32,32 --data-root /cifar100 --warmup 0
    echo "Run ${i} done"
done
