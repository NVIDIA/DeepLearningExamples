...
def get_categorical_feature_sizes(FLAGS):
    ...
    categorical_sizes_file = os.path.join("./notebooks/model_size.json")
...
flags.DEFINE_integer("num_numerical_features", 171, #v4 prep
...
def main(argv):
    ...
    data_loader_train, data_loader_test = None, None  #get_data_loaders(FLAGS)
...
def train(model, loss_fn, optimizer, data_loader_train, data_loader_test, scaled_lr):
    ...
    loss, auc, test_step_time = evaluate(model, loss_fn, test_numerical_features,
    ...
    for epoch in range(FLAGS.epochs):
        ...
        #form training data from ratings
        train_data = train_data.reindex(np.random.permutation(train_data.index))
        batch_indices = np.array_split(train_data.index, steps_per_epoch)
        ...
        for step in range(steps_per_epoch):
            num_samples = len(batch_indices[step])

            batch_data = train_data.ix[batch_indices[step]].copy()
            batch_data_neg = batch_data.copy()
            batch_data_neg['movieId'] = all_movies_id.sample(n=batch_data_neg.shape[0], replace=True).values
            batch_data = pd.concat([batch_data, batch_data_neg])

            batch_data = batch_data.merge(movie_data, on='movieId', how='left')

            numerical_features = torch.tensor(batch_data[num_feats].values, dtype=torch.float32, device='cuda')
            categorical_features = torch.tensor(batch_data[cat_feats].values, dtype=torch.int64, device='cuda')
            click = torch.cat((torch.ones((num_samples), dtype=torch.float32, device='cuda'), torch.zeros((num_samples), dtype=torch.float32, device='cuda')), 0)
...
def evaluate(model, loss_fn, test_numerical_features, test_categorical_features, test_click):
    """Test dlrm model
    ...
    model.eval()
    with torch.no_grad():
        output = model(test_numerical_features, test_categorical_features).squeeze()

        #pdb.set_trace()
        loss = loss_fn(output, test_click)
        y_true = test_click
        y_score = output
