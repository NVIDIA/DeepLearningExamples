...
live_command="curl -m 1 -L -s -o /dev/null -w %{http_code} localhost:8000/v2/health/live"
ready_command="curl -m 1 -L -s -o /dev/null -w %{http_code} localhost:8000/v2/health/ready"
...
while [[ ${current_status} != ${SUCCESS_CODE} ]] || [[ $($ready_command) != ${SUCCESS_CODE} ]]; do
