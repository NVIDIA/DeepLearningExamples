...
from ft_tensorflow_quantization import get_calibrators
...
flags.DEFINE_string("calib_method", "percentile", "calibration method [percentile, mse]")
...
flags.DEFINE_bool("quantize_residual", True, "Whether to quantize.")
flags.DEFINE_bool("quant_another_add_input", False, "Whether to quantize another add&layernorm input except residual connection.")
if FLAGS.quantize_residual:
  modeling.QUANT_RESIDUAL = True
else:
  modeling.QUANT_RESIDUAL = False
if FLAGS.quant_another_add_input:
  modeling.QUANT_ANOTHER_ADD_INPUT = True
else:
  modeling.QUANT_ANOTHER_ADD_INPUT = False
  ...
  def begin(self):
    ...
    for _, calib_list in self.calibrator_lists.items():
      for calibrator in calib_list:
        ...
        self.load_min_op[calibrator.tensor_name_prefix] = tf.compat.v1.assign(graph.get_tensor_by_name(calibrator.quant_min_name), self.placeholders[calibrator.tensor_name_prefix])
        self.load_max_op[calibrator._tensor_name_prefix] = tf.compat.v1.assign(graph.get_tensor_by_name(calibrator.quant_max_name), self.placeholders[calibrator.tensor_name_prefix])
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    tvars = tf.trainable_variables()
    tvars = tvars + tf.get_collection("quantization_variables")
...
def main(_):
  ...
  if FLAGS.do_calib:
    training_hooks.append(CalibrationHook())
