  ...
  def compute_range(self, **kwargs):
    ...
    Keyword Arguments:
      ...
      start_bin: An integer. Histogram bin to start sweep. Default 128.
          Only needed when `calibration_method == "mse"`.
    ...
    if isinstance(self._collector, MaxCollector):
      ...
      if self._collector.calib_bin_edges is None or self._collector.calib_hist is None:
        raise RuntimeError("The collector have not collected anything, cannot compute the range.")
      calibration_method = kwargs.pop("calibration_method", None)
      if calibration_method is None:
        raise ValueError("calibration_method should be passed when use HistogramCollector")
      ...
      if calibration_method == 'percentile':
        ...
        raise ValueError("calibration_method must be one of ['percentile', 'mse']")
...
def get_calibrators(collection_name_prefix,
  ...
  Args:
    ...
    scope_name: A string. The scope_name used in :func:`FakeQuantizer <quantization.FakeQuantizer>`.
  ...
  for i in range(collection_size):
    assert scope_name in qmin_collection[i]
    assert scope_name in qmax_collection[i]
    assert scope_name in calib_tensor_collection[i]
