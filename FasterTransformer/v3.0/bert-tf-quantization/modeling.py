...
from ft_tensorflow_quantization import FakeQuantizer, QuantDense, QuantDescriptor
...
class BertModel(object):
  ...
  def __init__(self,
	...
	# it is important that the embedding codes remain fp32.
      ...
      output_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
      self.sequence_output = tf.cast(output_quantizer(self.all_encoder_layers[-1]), tf.float32)
...
def attention_layer(from_tensor,
  ...
  query_layer = transpose_for_scores(query_layer, batch_size,
                                     ...
                                     size_per_head)
  query_layer_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
  query_layer = query_layer_quantizer(query_layer)

  # `key_layer` = [B, N, T, H]
  key_layer = transpose_for_scores(key_layer, batch_size, num_attention_heads,
                                   to_seq_length, size_per_head)
  key_layer_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
  ...
  value_layer_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
  value_layer = value_layer_quantizer(value_layer)
  attention_probs_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
    ...
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      with tf.variable_scope("attention"):
        ...
        with tf.variable_scope("output"):
          ...
          if QUANT_RESIDUAL:
            layer_input_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
            layer_input = layer_input_quantizer(layer_input)
          if QUANT_ANOTHER_ADD_INPUT:
            attention_output_quantizer_2 = FakeQuantizer(INPUT_DESC, if_quant)
            attention_output = attention_output_quantizer_2(attention_output)
      ...
      with tf.variable_scope("output"):
        ...
        if QUANT_RESIDUAL:
          attention_output_quantizer_4 = FakeQuantizer(INPUT_DESC, if_quant)
          attention_output = attention_output_quantizer_4(attention_output)
        if QUANT_ANOTHER_ADD_INPUT:
          layer_output_quantizer = FakeQuantizer(INPUT_DESC, if_quant)
