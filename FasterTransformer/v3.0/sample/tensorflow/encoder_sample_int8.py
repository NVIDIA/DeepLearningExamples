...
of FasterTransformer (fp32/fp16). 

Users are also able to use this sample code to test the average forward time of 
TensorFlow and FasterTransformer (fp32/fp16 and int8 with quantizing residual and int8 without quantizing residual). 
'''
...
if __name__ == "__main__":
    ...
    op_encoder_notInt8_result = op_encoder(inputs=from_tensor,
                                   encoder_args=encoder_notInt8_args,
                                   attention_mask=attention_mask,
                                   encoder_vars_dict=encoder_variables_dict,
                                   sequence_length=sequence_length)

    op_encoder_int8_v1_result = op_encoder(inputs=from_tensor,
                                   encoder_args=encoder_Int8_v1_args,
                                   attention_mask=attention_mask,
                                   encoder_vars_dict=encoder_variables_dict,
                                   sequence_length=sequence_length)
                                   
    op_encoder_int8_v2_result = op_encoder(inputs=from_tensor,
    ...
    op_encoder_notInt8_result2 = op_encoder_notInt8_result * tf.expand_dims(tf.sequence_mask(sequence_length, maxlen=max_seq_len, dtype=tf_datatype), axis=-1)
    ...
    with tf.Session(config=config) as sess:
        ...
        op_encoder_notInt8_result_val = sess.run(op_encoder_notInt8_result2)
        
        cross_check("Encoder TF v.s. FT with tensor input", tf_encoder_result_val,
                    op_encoder_notInt8_result_val, atol_threshold)
        ...
        if args.test_time == 1:
            ...
            #time.sleep(25)
            op_encoder_int8_v1_time = time_test(sess, op_encoder_int8_v1_result, ite)
            #time.sleep(25)
            op_encoder_int8_v2_time = time_test(sess, op_encoder_int8_v2_result, ite)
            #time.sleep(25)
            op_encoder_notInt8_time = time_test(sess, op_encoder_notInt8_result, ite)
            ...
            print("[INFO] batch_size {} max_seq_len {} {} layer FT-OP-tensor-time {:6.2f} ms".format(batch_size, max_seq_len, num_layer, op_encoder_notInt8_time))
            print("[INFO] batch_size {} max_seq_len {} {} layer FT-INT8v1-OP-tensor-time {:6.2f} ms".format(batch_size, max_seq_len, num_layer, op_encoder_int8_v1_time))
            print("[INFO] batch_size {} max_seq_len {} {} layer FT-INT8v2-OP-tensor-time {:6.2f} ms".format(batch_size, max_seq_len, num_layer, op_encoder_int8_v2_time))
