...
class Subtokenizer(object):
  ...
  def __init__(self, vocab_file, reserved_tokens=None):
    """Initializes class based on given vocab file.
    ...
    Args:
      ...
      reserved_tokens: List of string tokens that are guaranteed to be at the
        beginning of the subtoken vocabulary list. If 'assumed_in_file', reserved_tokens
  ...
  def init_from_files(
    ...
    if os.path.exists(vocab_file):
      ...
      alphabet = _generate_alphabet_dict(token_counts, reserved_tokens)
    ...
    return Subtokenizer(vocab_file, reserved_tokens)

  def eos_id(self):
      return EOS_ID

  def encode_as_ids(self, raw_string, add_eos=False):
...
def _count_tokens(files, file_byte_limit=1e6):
  ...
  for filepath in files:
    with open(filepath, mode='r', newline='\n') as reader:
      ...
      size_reader = reader.tell()
      reader.seek(0,0)
      lines_to_skip = int(size_reader / (file_byte_budget * 2))
      for line in reader:
...
def _generate_subtokens_with_target_vocab_size(
  ...
  if min_count is not None:
    ...
    subtokens = _generate_subtokens(
        token_counts, alphabet, min_count, reserved_tokens=reserved_tokens)
     # Trim vocabulary to declared size
    if len(subtokens)>target_size:
        for i in range(len(subtokens)-1, 0, -1):
            if subtokens[i] not in alphabet:
               del subtokens[i]
            if len(subtokens) == target_size:
                break
    return subtokens
