...
from model import fp16_utils
from model import fused_layer_norm
...
class Transformer(object):
  ...
  def __init__(self, params, train):
    ...
    self.embedding_softmax_layer = embedding_layer.EmbeddingSharedWeights(params)
    self.encoder_stack = EncoderStack(params, train)
    self.decoder_stack = DecoderStack(params, train)

    # Component precisions
    self.inputs_padding_dtype = self.params.dtype
    self.attention_bias_dtype = tf.float32
    self.decoder_self_attention_bias_dtype = tf.float32
  ...
  def __call__(self, inputs, targets=None):
    ...
    with tf.variable_scope("Transformer", initializer=initializer):
      ...
      # Generate output sequence if targets is None, or return logits if target sequence is known.
      if targets is None:
  ...
  def encode(self, inputs, attention_bias):
    ...
    with tf.name_scope("encode"):
      ...
      inputs_padding = model_utils.get_padding(inputs, compute_type=self.inputs_padding_dtype)

      with tf.name_scope("add_pos_encoding"):
        length = tf.shape(embedded_inputs)[1]
        pos_encoding = model_utils.get_position_encoding(
            length, self.params.hidden_size, compute_type=self.params.dtype)
  ...
  def decode(self, targets, encoder_outputs, attention_bias):
    ...
    with tf.name_scope("decode"):
      ...
      decoder_self_attention_bias = model_utils.get_decoder_self_attention_bias(
          length, compute_type=self.decoder_self_attention_bias_dtype)
...
class LayerNormalization(tf.layers.Layer):
  ...
  def call(self, x, epsilon=1e-6):
    ...
    return fused_layer_norm.fused_layer_norm(x, begin_norm_axis=-1, use_fused_batch_norm=True)
...
class EncoderStack(tf.layers.Layer):
  ...
  def __init__(self, params, train):
    ...
    for _ in range(params.num_hidden_layers):
      ...
      self_attention_layer = attention_layer.SelfAttention(params, train)
      feed_forward_network = ffn_layer.FeedFowardNetwork(params, train)
  ...
  def call(self, encoder_inputs, attention_bias, inputs_padding):
    for n, layer in enumerate(self.layers):
      ...
      if self.params.dtype != tf.float32:
        ...
        inputs_padding = tf.cast(inputs_padding, self.params.dtype)
...
class DecoderStack(tf.layers.Layer):
  ...
  def __init__(self, params, train):
    ...
    for _ in range(params.num_hidden_layers):
      self_attention_layer = attention_layer.SelfAttention(params, train)
      enc_dec_attention_layer = attention_layer.Attention(params, train)
      feed_forward_network = ffn_layer.FeedFowardNetwork(params, train)
  ...
  def call(self, decoder_inputs, encoder_outputs, decoder_self_attention_bias, attention_bias, cache=None):
    for n, layer in enumerate(self.layers):
      ...
      # cast inputs to compute type
      if self.params.dtype != tf.float32:
        decoder_inputs = tf.cast(decoder_inputs, self.params.dtype)
        decoder_self_attention_bias = tf.cast(decoder_self_attention_bias, self.params.dtype)
        attention_bias = tf.cast(attention_bias, self.params.dtype)
