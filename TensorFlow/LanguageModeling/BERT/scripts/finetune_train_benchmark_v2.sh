    ...
    for seq_len in 128 384; do
        ...
        for batch_size in 1 2 4; do
            for precision in fp16 fp32; do
                ...
                perfwo=`cat $tmp_file | grep -F 'Throughput Average (sentences/sec) with overhead =' | awk -F'= ' '{print $2}'`
                TTTwo=`cat $tmp_file | grep -F 'Total Training Time W/O Overhead =' | awk -F'= ' '{print $2}'`
                TTT=`cat $tmp_file | grep -F 'Total Training Time =' | awk -F'= ' '{print $2}'`

                echo "$precision $seq_len  $batch_size $perf $perfwo $TTTwo $TTT" >> $LOGFILE
