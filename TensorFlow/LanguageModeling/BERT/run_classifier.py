...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if not is_training and FLAGS.use_trt:
            trt_graph = get_frozen_tftrt_model(bert_config, input_ids.shape, num_labels, use_one_hot_embeddings,
                                               init_checkpoint)
            (total_loss, per_example_loss, logits, probabilities) = tf.import_graph_def(trt_graph,
                                                                                        ...
                                                                                        return_elements=[
                                                                                            'loss/cls_loss:0',
                                                                                            'loss/cls_per_example_loss:0',
                                                                                            'loss/cls_logits:0',
                                                                                            'loss/cls_probabilities:0'],
                                                                                        name='')
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        ss_sentences_per_second = (num_train_steps - training_hooks[
            -1].skipped) * global_batch_size * 1.0 / train_time_wo_overhead
